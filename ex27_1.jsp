...
// 	MultipartRequest mr = new MultipartRequest(request,"C:\\myhome2\\FAQ\\src\\main\\webapp\\upload",1024 * 1024 * 2,"UTF-8",new DefaultFileRenamePolicy());
	...
	// new DefaultFileRenamePolicy() : 새로운 객체 생성을 하기 위해 사용하는 
	/*	Enumeration : 파일 객체 이므로 자료형으로 받음	*/
	Enumeration files = mr.getFileNames();	// getFileNames() : 배열 정보
	/*	문자열로 변환 nextElement() : 업로드한 파일 모두를 지칭	*/
...
// 		out.print(fnm);
		// abc.jpg -> abc jpg
		// IndexOf : 앞에서 부터 단어를 읽어감
		// lastIndexOf  : 뒤에서 부터 단어를 읽어감
		// substring : 글자를 자를 때 사용하게 됨
		...
		// StandardCopyOption.ATOMIC_MOVE : JAJA언어
		// 기존파일은 삭제를 하고, 변경된 파일로 복사하면서 이름이 변경

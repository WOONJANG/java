...
/*
 	...
 	JSON 라이브러리를 이용하여 해당 api 배열 데이터를 키값으로 분리 되도록 한다.
 	출력 확인 후 Controller로 변경
 	단, 변경시 주의 사항은 객체형 메소드 main 메소드를 변경해야 한다.
 	그 외에 @Autowired, @RequestMapping 이용해서 웹페이지로 실행 시킨 후 DB 입력값을 확인 하게 된다.
...
//				System.out.println(as.size());
				...
				int w = 0;
				while (w < as.size()) {
					JSONObject jobject = (JSONObject)as.get(w);
					
					String datain = "insert into apimovie values ('0',?,?,?,?,?)";
					PreparedStatement ps = con.prepareStatement(datain);
